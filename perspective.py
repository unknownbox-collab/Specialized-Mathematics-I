...
class Line:
    ...
    def draw(self,screen):
        pygame.draw.line(screen, RED, (self.x, self.y), (self.x + DISTANCE, self.y + DISTANCE*self.incline), 1)
        pygame.draw.line(screen, RED, (self.x, self.y), (self.x - DISTANCE, self.y - DISTANCE*self.incline), 1)
    ...
    def step3(self):
        ...
        if varphi is not None:
            ...
            if alpha is not None:
                a = self.x2 + self.r2 * math.cos(varphi)/math.sin(alpha)*abs(self.x1 - self.x2)/(self.x1 - self.x2)
                b = self.y2 + self.r2 * math.sin(varphi)/math.sin(alpha)*abs(self.x1 - self.x2)/(self.x1 - self.x2)
                return (a,b)
